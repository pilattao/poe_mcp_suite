...
- [x] Test a real-shaped PathOfBuilding2 fixture with independent SkillSet/ItemSet, two weapons, charms, runes and WeaponSet1/2 node lists. Assert selected groups and round-trip retention.
- [x] Test extensionless build names, .xml suffix, nested paths, Windows separators and traversal rejection.
- [x] Introduce a shared XML root reader used by all handlers, retaining the original game root on writes.
- [x] Resolve installed root and source root layouts; normalize PoB2 connections and explicit numeric Lua tables to usable arrays.
- [x] Assert the real installed 0_5 tree finds existing character nodes and paths; never select PoE1 tree as fallback.
- [x] Run baseline regressions and new contract tests; commit this independently.
...
- [x] Fork PoB2 source and graft the adapter with upstream provenance.
...
- [x] Add a reversible parameterized installer supporting the supplied path and source/install directory layouts.
- [x] Start an isolated API-enabled PoB2 instance on a dedicated loopback port; verify ping, version, read build and calculated stats.
- [x] Exercise item, gem, config, level and weapon-specialisation changes; restore and compare complete original XML and calculated baseline.
...
- [x] Add explicit game/league/source selection and installed PoB2 data path.
- [x] Test gems, supports, uniques, base items, mods, trees, atlas and crafting using PoE2 exemplars.
- [ ] Port PoB parsing and comparisons to PathOfBuilding2.
- [x] Test prices and currencies against supported PoE2 economy responses and selected league.
...
- [x] Test PoE2 query schemas, item categories, stat IDs, currency units, realm and league selection.
- [x] Integrate the already-working public character browser export as a dated snapshot source; preserve official OAuth character access as a separate source.
- [x] Validate protected inventory/stash paths against current official capability; identify exact access requirements before asking the user.
...
- [x] Surface registration/import errors instead of silently dropping modules.
...
- [x] Configure all servers for the user's Windows/WSL paths, correct game and league.
...
- [x] Push child fork commits before suite pointers; verify a clean clone can reproduce setup.
